...
static ALint format = AL_FORMAT_STEREO16;
static ALuint dwSampleRate = 44100;
static int    do_quit = 0;

#define NUMBUFFERS              (3)
static ALuint	uiBuffers[NUMBUFFERS] = { -1,-1,-1 };
static ALuint   source=-1;

extern void printf_msg(const char* msg);
...
typedef struct PacketQueue {
    ...
    int abort_request;
    int serial;
	Linux_Win_Lock* mutex;
	Linux_Win_Event* cond;
    //SDL_mutex *mutex;
    //SDL_cond *cond;
} PacketQueue;
...
typedef struct Decoder {
    ...
    int finished;
    int packet_pending;
	Linux_Win_Event *empty_queue_cond;
    int64_t start_pts;
    AVRational start_pts_tb;
    int64_t next_pts;
    AVRational next_pts_tb;
    Linux_Win_Event *decoder_tid;
} Decoder;

typedef struct VideoState {
	Linux_Win_Event *read_tid;
	...
	Linux_Win_Event *continue_read_thread;
} VideoState;

/* options specified by the user */
static VideoState *current_is = NULL;
...
static const char* wanted_stream_spec[AVMEDIA_TYPE_NB] = {"vst","ast",0,"sst",0};
...
static enum ShowMode show_mode = (ShowMode)-1;
...
static void decoder_init(Decoder *d, AVCodecContext *avctx, PacketQueue *queue, Linux_Win_Event *empty_queue_cond) {
    ...
    if (show_status) {
        ...
        if (!last_time || (cur_time - last_time) >= 30000) {
            ...
            av_log(NULL, AV_LOG_INFO,
                   "%7.2f %s:%7.3f fd=%4d aq=%5dKB vq=%5dKB sq=%5dB f=%II/%II\"   \r",
                   get_master_clock(is),
...
static void alloc_picture(VideoState *is)
{
	...
	video_open(is, 0, vp);
	if (vp->bmp == NULL){
		vp->bmp = new _call_vframe;
		vp->bmp->width = vp->width;
		vp->bmp->height = vp->height;
		vp->bmp->data = (uint8_t*)new RGBA[vp->width*vp->height];
		vp->allocated = 1;
	}
}


static int queue_picture(VideoState *is, AVFrame *src_frame, AVFrame *frameRGBA, double pts, double duration, int64_t pos, int serial)
{
    Frame *vp;
    ...
    //    SDL_Event event;
       ...
       // SDL_PushEvent(&event);
        ...
        while (!vp->allocated && !is->videoq.abort_request) {
            SDL_CondWait2(is->pictq.cond, is->pictq.mutex);
        }
    ...
    if (vp->bmp) {
...
#else
        {
            ...
            if (e) {
                const AVClass *class_ = sws_get_class();
				const AVOption    *o = av_opt_find(&class_, "sws_flags", NULL, 0,
                                                   AV_OPT_SEARCH_FAKE_OBJ);
				int ret = av_opt_eval_flags(&class_, o, e->value, &sws_flags);
                if (ret < 0)
                    exit(1);
            }
        }
		...
		avpicture_fill((AVPicture *)frameRGBA, vp->bmp->data, AV_PIX_FMT_RGBA, vp->width, vp->height);

        sws_scale(is->img_convert_ctx, src_frame->data, src_frame->linesize,
			0, vp->height, frameRGBA->data, frameRGBA->linesize);
...
#endif
    av_frame_free(&frame);
	is->auddec.decoder_tid->SetEvent();
    return ret;
}
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
			...
			ret = queue_picture(is, frame, frameRGBA, pts, duration, av_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
           			
			if (frame->pts != AV_NOPTS_VALUE){
				int d = frame->pts * 10000 / is->video_st->duration;
				char buf[10];
				itoa(d, buf, 10);
				LXZSystem_DelayNotify("seekpos", buf, m_ctx, false);
			}
...
#endif
    av_frame_free(&frame);
	is->viddec.decoder_tid->SetEvent();
    return 0;
}
...
static int subtitle_thread(void *arg)
{
	...
	is->subdec.decoder_tid->SetEvent();
    return 0;
}
...
static int audio_decode_frame(VideoState *is)
{
    ...
    if (af->frame->format        != is->audio_src.fmt            ||
		...
		int err = 0;
        if (!is->swr_ctx || (err=swr_init(is->swr_ctx)) < 0) {
			char str[256];
			const char* errstr = av_make_error_string(str, 256, err);
            av_log(NULL, AV_LOG_ERROR,
                   "Cannot create sample rate converter for conversion of %d Hz %s %d channels to %d Hz %s %d channels! %s\n",
				   af->frame->sample_rate, av_get_sample_fmt_name((AVSampleFormat)af->frame->format), av_frame_get_channels(af->frame),
				   is->audio_tgt.freq, av_get_sample_fmt_name(is->audio_tgt.fmt), is->audio_tgt.channels, errstr);
            swr_free(&is->swr_ctx);
            return -1;
        }
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
	...
	LXZAPI_NewThread(ThreadPlayAudio, opaque);
	return is->audio_tgt.frame_size;    
}
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
		...
		if (av_get_bytes_per_sample(is->audio_tgt.fmt) == 1)
		{
			if (nb_channels == 1)
		...
		else if (av_get_bytes_per_sample(is->audio_tgt.fmt) == 2)
		{
			if (nb_channels == 1)
		...
		is->audio_tgt.frame_size = avctx->frame_size;
		is->audio_tgt.freq = 44100;
		is->audio_tgt.channel_layout = av_get_default_channel_layout(avctx->channels);
...
static void refresh_loop_wait_event(VideoState *is) {
   ...
   // SDL_PumpEvents();
    while (/*!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_ALLEVENTS)*/!is->abort_request) {
...
{
	double d = parse_number_or_die(opt, arg, OPT_INT, 0, VideoState::ShowMode::SHOW_MODE_NB - 1);
	enum ShowMode dd = (ShowMode)(int)d;
	if (!strcmp(arg, "video")){
		show_mode = (ShowMode)VideoState::SHOW_MODE_VIDEO;
	}
	else if (!strcmp(arg, "waves")){
		show_mode = (ShowMode)VideoState::SHOW_MODE_WAVES;
	}
	else if (!strcmp(arg, "rdft")){
		show_mode = (ShowMode)VideoState::SHOW_MODE_RDFT;
	}
	else{
		show_mode = dd;
	}
   ...
   switch(op) {
      case AV_LOCK_CREATE:
          *mtx = SDL_CreateMutex();
          ...
          return 0;
      case AV_LOCK_OBTAIN:
          return !!SDL_LockMutex((Linux_Win_Lock*)*mtx);
      case AV_LOCK_RELEASE:
		  return !!SDL_UnlockMutex((Linux_Win_Lock*)*mtx);
      case AV_LOCK_DESTROY:
		  SDL_DestroyMutex((Linux_Win_Lock*)*mtx);
          return 0;
   }
   return 1;
}*/
...
extern "C"{
	int ThreadRefreshMain(void* data){
...
void ccPlay(const char* param, ILXZAlloc* strResult){
	m_ctx = LXZAPI_GetNowLaeContext();

	if (current_is){
		stream_close(current_is);
		current_is = NULL;
	}
	...
	is = stream_open(param, file_iformat);
	if (!is) {
		av_log(NULL, AV_LOG_FATAL, "Failed to initialize VideoState!\n");
		do_exit(NULL);
	}
	...
	SDL_CreateThread(ThreadRefreshMain, is);
	
}

void ccSeekpos(const char* param, ILXZAlloc* strResult){
	//playCtrl.state = STATE_STOP;
	if (current_is) {		
		int a = atoi(param);
		uint64_t size = avio_size(current_is->ic->pb);
		stream_seek(current_is, size*a/100, 0, 1);
	}
}

void ccResume(const char* param, ILXZAlloc* strResult){
	//playCtrl.state = STATE_PLAY;	
	if (current_is){
		stream_toggle_pause(current_is);
	}
}


void ccPause(const char* param, ILXZAlloc* strResult){
...
void DoIncr(int incr){
	if (seek_by_bytes) {
		double pos = -1;
		if (pos < 0 && current_is->video_stream >= 0)
			pos = frame_queue_last_pos(&current_is->pictq);
		if (pos < 0 && current_is->audio_stream >= 0)
			pos = frame_queue_last_pos(&current_is->sampq);
		if (pos < 0)
			pos = avio_tell(current_is->ic->pb);
		if (current_is->ic->bit_rate)
			incr *= current_is->ic->bit_rate / 8.0;
		else
			incr *= 180000.0;
		pos += incr;
		stream_seek(current_is, pos, incr, 1);
	}
	else {
		double pos = get_master_clock(current_is);
		if (isnan(pos))
			pos = (double)current_is->seek_pos / AV_TIME_BASE;
		pos += incr;
		if (current_is->ic->start_time != AV_NOPTS_VALUE && pos < current_is->ic->start_time / (double)AV_TIME_BASE)
			pos = current_is->ic->start_time / (double)AV_TIME_BASE;
		stream_seek(current_is, (int64_t)(pos * AV_TIME_BASE), (int64_t)(incr * AV_TIME_BASE), 0);
	}
}

void ccSkipNext(const char* param, ILXZAlloc* strResult){
	if (current_is->ic->nb_chapters <= 1) {
		DoIncr(600.0);
	}
	else{
		seek_chapter(current_is, 1);
	}
}

void ccSkipBack(const char* param, ILXZAlloc* strResult){
	if (current_is == NULL)
		return;
...
void ccMaxSize(const char* param, ILXZAlloc* strResult){
//	screen_width = cur_stream->width = screen->w;
//	screen_height = cur_stream->height = screen->h;
//	current_is->force_refresh = 1;
}

void ccMutex(const char* param, ILXZAlloc* strResult){
	if (current_is){
		toggle_mute(current_is);
		if (current_is->muted){
			alSourcef(source, AL_GAIN, 0);
		}
		else{
			alSourcef(source, AL_GAIN, current_is->audio_volume / (float)SDL_MIX_MAXVOLUME);
		}
	}
}

void ccSetVolume(const char* param, ILXZAlloc* strResult){
	if (current_is == NULL)
		return;
		
	current_is->audio_volume = av_clip(atoi(param), 0, SDL_MIX_MAXVOLUME);
	//update_volume(current_is, );
}